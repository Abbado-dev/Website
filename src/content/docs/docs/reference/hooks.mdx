...
description: Complete reference for all hook events ia-ide handles, what they do, and the hook script format.
---
...
ia-ide checks for the event name in these fields (in order): `hook_event_name`, `event_name`, `hookEventName`.
...
2. POSTs it to the ia-ide hook endpoint
...
- This is how ia-ide transitions from `queued` or `paused` to `running`
...
<Aside type="note">
ia-ide intentionally does not auto-approve permissions. The interactive terminal lets the user review and approve/deny each request manually.
</Aside>
...
2. Watch the backend logs for `hook received` messages:
   ```
   INFO ia_ide_api::runs: hook received run_id=a1b2c3d4 event=UserPromptSubmit
   INFO ia_ide_api::runs: hook received run_id=a1b2c3d4 event=PreToolUse
   ```
