...
| `ABBADO_BIND` | `127.0.0.1:4320` | Address and port the server binds to |
...
```bash
ABBADO_BIND=0.0.0.0:8080 cargo run
```
...
The frontend dev server (Vite) proxies API requests to `http://localhost:4320`. This is configured in the Vite config and requires no manual setup.
...
Abbado binds to `127.0.0.1:4320` by default — localhost only. To expose it on the network:

```bash
ABBADO_BIND=0.0.0.0:4320 cargo run
```

<Aside type="caution">
Abbado has no authentication. Do not expose it to the public internet. Use it on localhost or behind a VPN/SSH tunnel.
</Aside>
