...
<Aside type="note">
Make sure you have ia-ide running. See [Installation](/docs/installation/) if you haven't set it up yet.
</Aside>
...
2. **Add a git repository**
   ...
   The repo must have at least one commit. ia-ide validates that the path exists and contains a `.git` directory.
...
4. **Launch a session**
   ...
   Click **"+ Session"** on the agent card. ia-ide will:
...
7. **Commit and push**
   ...
   You can also rename the branch before pushing if the auto-generated name (`ia-ide/run-XXXX`) is not descriptive enough.

</Steps>

## What Just Happened

Behind the scenes, ia-ide:

1. Created a **git worktree** from your repo's HEAD, giving the agent an isolated branch (`ia-ide/run-XXXX`)
