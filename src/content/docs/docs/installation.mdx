---
title: Installation
description: Prerequisites, step-by-step installation, and environment variables for ia-ide.
---
...
```bash
git clone https://github.com/Raznak/ia-ide.git
cd ia-ide
```
...
| `RUST_LOG` | `ia_ide=debug` | Log level (uses `tracing` crate) |

Example: run on a different port with a custom data directory:

```bash
IA_IDE_BIND=0.0.0.0:8080 IA_IDE_DATA_DIR=/opt/ia-ide/data cargo run
```
...
2. You should see the ia-ide dashboard with an empty sidebar
