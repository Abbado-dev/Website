...
description: How Abbado integrates with Claude Code and Codex hooks to track agent activity in real-time.
---
...
```bash
#!/bin/bash
RESPONSE=$(curl -s -X POST "http://127.0.0.1:4320/api/runs/<run-id>/hook" \
